...
CREATE UNIQUE INDEX uk_users_student_id ON users (student_id)
WHERE
...
ADD CONSTRAINT chk_rider_status CHECK (
...
ALTER TABLE driver_profiles
ADD CONSTRAINT chk_commission_rate CHECK (
    commission_rate >= 0
    AND commission_rate <= 1
);

ALTER TABLE driver_profiles
ADD CONSTRAINT chk_rating_avg_driver CHECK (
    rating_avg >= 0
    AND rating_avg <= 5
);
...
ADD CONSTRAINT chk_payment_method CHECK (
    preferred_payment_method IN ('WALLET', 'CREDIT_CARD')
);
...
ALTER TABLE wallets
ADD CONSTRAINT chk_balance_positive CHECK (shadow_balance >= 0);

ALTER TABLE wallets
ADD CONSTRAINT chk_pending_positive CHECK (pending_balance >= 0);
...
ADD CONSTRAINT chk_verification_type CHECK (
    type IN (
        'STUDENT_ID',
        'DRIVER_LICENSE',
        'BACKGROUND_CHECK',
...
ADD CONSTRAINT chk_fuel_type CHECK (
    fuel_type IN ('GASOLINE', 'ELECTRIC')
);

ALTER TABLE vehicles
ADD CONSTRAINT chk_vehicle_year CHECK (
    year >= 2000
    AND year <= EXTRACT(
        YEAR
        FROM CURRENT_DATE
...
ALTER TABLE vehicles
ADD CONSTRAINT chk_vehicle_capacity CHECK (
    capacity >= 1
    AND capacity <= 2
);
...
SELECT dp.driver_id, '29A-12345', 'Honda Wave Alpha', 'Black', 2022, 1, 2, '2025-01-01 00:00:00', '2024-06-01 00:00:00', 'GASOLINE', 'ACTIVE', '2024-06-10 12:00:00'
...
SELECT dp.driver_id, '29A-12344', 'Honda Wave Alpha', 'Black', 2022, 1, 2, '2025-01-01 00:00:00', '2024-06-01 00:00:00', 'GASOLINE', 'ACTIVE', '2024-06-10 12:00:00'
FROM driver_profiles dp
    JOIN users u ON dp.driver_id = u.user_id
WHERE
...
ADD CONSTRAINT chk_lat_range CHECK (
    lat >= -90
    AND lat <= 90
);

ALTER TABLE locations
ADD CONSTRAINT chk_lng_range CHECK (
    lng >= -180
    AND lng <= 180
);
...
ADD CONSTRAINT chk_shared_rides_status CHECK (
    status IN (
        'SCHEDULED',
        'ACTIVE',
        'COMPLETED',
...
ALTER TABLE shared_rides
ADD CONSTRAINT chk_max_passengers CHECK (
    max_passengers >= 1
    AND max_passengers <= 3
);

ALTER TABLE shared_rides
ADD CONSTRAINT chk_current_passengers CHECK (
    current_passengers >= 0
    AND current_passengers <= max_passengers
);

ALTER TABLE shared_rides
ADD CONSTRAINT chk_base_fare CHECK (base_fare >= 0);

ALTER TABLE shared_rides
ADD CONSTRAINT chk_per_km_rate CHECK (per_km_rate >= 0);
...
ADD CONSTRAINT chk_discount_type CHECK (
    discount_type IN ('PERCENTAGE', 'FIXED_AMOUNT')
);

ALTER TABLE promotions
ADD CONSTRAINT chk_target_user_type CHECK (
    target_user_type IN ('RIDER', 'DRIVER', 'ALL')
    OR target_user_type IS NULL
);

ALTER TABLE promotions
ADD CONSTRAINT chk_valid_dates CHECK (valid_until > valid_from);

ALTER TABLE promotions
ADD CONSTRAINT chk_usage_limits CHECK (
    usage_limit IS NULL
    OR usage_limit > 0
);

ALTER TABLE promotions
ADD CONSTRAINT chk_usage_limit_per_user CHECK (
...
ADD CONSTRAINT chk_shared_ride_request_status CHECK (
    status IN (
        'PENDING',
        'CONFIRMED',
        'IN_PROGRESS',
...
ALTER TABLE shared_ride_requests
ADD CONSTRAINT chk_fare_amount CHECK (fare_amount >= 0);

ALTER TABLE shared_ride_requests
ADD CONSTRAINT chk_original_fare CHECK (
    original_fare IS NULL
    OR original_fare >= 0
);

ALTER TABLE shared_ride_requests
ADD CONSTRAINT chk_discount_amount CHECK (
    discount_amount IS NULL
    OR discount_amount >= 0
);

ALTER TABLE shared_ride_requests
ADD CONSTRAINT chk_max_wait_time CHECK (
    max_wait_time IS NULL
    OR max_wait_time > 0
);
...
ADD CONSTRAINT chk_search_radius CHECK (
    search_radius_km IS NULL
    OR search_radius_km > 0
);

ALTER TABLE ai_matching_logs
ADD CONSTRAINT chk_available_drivers_count CHECK (
    available_drivers_count IS NULL
    OR available_drivers_count >= 0
);

ALTER TABLE ai_matching_logs
ADD CONSTRAINT chk_matching_score CHECK (
...
ALTER TABLE ai_matching_logs
ADD CONSTRAINT chk_processing_time CHECK (
...
CREATE TABLE transactions (
    ...
    CONSTRAINT txn_system_wallet_presence CHECK (
        (
            actor_kind = 'SYSTEM'
            AND system_wallet IS NOT NULL
        )
        OR (
            actor_kind <> 'SYSTEM'
            AND system_wallet IS NULL
        )
    ),
    CONSTRAINT txn_snapshots_only_for_user CHECK (
        (actor_kind = 'USER')
        OR (
            before_avail IS NULL
            AND after_avail IS NULL
            AND before_pending IS NULL
            AND after_pending IS NULL
        )
    ),
    ...
    CONSTRAINT txn_capture_role_alignment CHECK (
        ...
        OR (
            (
                actor_kind = 'USER'
                AND direction = 'OUT'
                AND rider_user_id = actor_user_id
            )
            OR (
                actor_kind = 'USER'
                AND direction = 'IN'
                AND driver_user_id = actor_user_id
            )
            OR (
                actor_kind = 'SYSTEM'
                AND system_wallet = 'COMMISSION'
                AND direction = 'IN'
            )
        )
    ),
...
CREATE INDEX idx_txn_user ON transactions (
    actor_kind,
    actor_user_id,
    status,
    created_at
...
CREATE INDEX idx_txn_system ON transactions (
    actor_kind,
    system_wallet,
    status,
    created_at
...
CREATE INDEX idx_txn_booking ON transactions (booking_id, created_at);
...
CREATE UNIQUE INDEX idx_emergency_contacts_user_primary ON emergency_contacts (user_id)
WHERE
    is_primary = true;
...
ALTER TABLE file_uploads
ADD CONSTRAINT chk_upload_status CHECK (
    upload_status IN (
        'UPLOADING',
        'UPLOADED',
        'FAILED',
...
ALTER TABLE messages
ADD CONSTRAINT chk_sender_receiver_different CHECK (sender_id != receiver_id);
...
ALTER TABLE ratings
ADD CONSTRAINT chk_rating_type CHECK (
    rating_type IN (
        'GENERAL',
        'RIDER_TO_DRIVER',
        'DRIVER_TO_RIDER'
...
ALTER TABLE ratings
ADD CONSTRAINT chk_safety_score_range CHECK (
...
ALTER TABLE ratings
ADD CONSTRAINT chk_punctuality_score_range CHECK (
    punctuality_score IS NULL
    OR (
        punctuality_score >= 1
        AND punctuality_score <= 5
    )
);

ALTER TABLE ratings
ADD CONSTRAINT chk_communication_score_range CHECK (
...
CREATE UNIQUE INDEX idx_ratings_unique_rating ON ratings (
    shared_ride_request_id,
    rater_id,
    target_id
);
...
ALTER TABLE sos_alerts
ADD CONSTRAINT chk_sos_status CHECK (
    status IN (
        'ACTIVE',
        'ACKNOWLEDGED',
        'RESOLVED',
...
ALTER TABLE sos_alerts
ADD CONSTRAINT fk_sos_alerts_triggered_by FOREIGN KEY (triggered_by) REFERENCES users (user_id) ON DELETE CASCADE;

ALTER TABLE sos_alerts
ADD CONSTRAINT fk_sos_alerts_acknowledged_by FOREIGN KEY (acknowledged_by) REFERENCES users (user_id) ON DELETE SET NULL;
...
ADD CONSTRAINT chk_discount_applied CHECK (
    discount_applied IS NULL
    OR discount_applied >= 0
);

-- Ensure one promotion usage per ride request
CREATE UNIQUE INDEX idx_user_promotions_unique_usage ON user_promotions (
    shared_ride_request_id,
    promotion_id
);