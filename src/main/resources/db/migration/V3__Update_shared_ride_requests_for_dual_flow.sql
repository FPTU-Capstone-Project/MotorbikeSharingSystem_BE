...
CHECK (
    -- BOOKING: must start with null shared_ride_id when PENDING
    (request_kind = 'BOOKING' AND status = 'PENDING' AND shared_ride_id IS NULL)
    OR
...
'Ensures data integrity: BOOKING starts without ride, gets assigned on accept. JOIN_RIDE always has ride. Terminal states flexible.';
...
'Optimizes matching algorithm queries for pending BOOKING requests by pickup time.';
