...
// import
// com.mssus.app.infrastructure.config.properties.RideConfigurationProperties;
...
// "${app.ride.auto-lifecycle.scan-interval-ms:60000}")
...
// log.debug("Skipping lifecycle enforcement run because previous execution is
// still in progress");
// return;
// }
...
// List<SharedRide> candidates =
// rideRepository.findScheduledForAutoStart(cutoff);
...
// List<SharedRideRequest> requests =
// requestRepository.findConfirmedForAutoStart(cutoff);
...
// request.getSharedRideRequestId(), ride.getSharedRideId(),
...
// LocalDateTime cutoff = now.minus(timeout);
// List<SharedRideRequest> requests =
// requestRepository.findOngoingForAutoCompletion(cutoff);
// for (SharedRideRequest request : requests) {
// SharedRide ride = request.getSharedRide();
// if (ride == null || ride.getStatus() != SharedRideStatus.ONGOING) {
// continue;
...
// try {
// Authentication automationAuth =
// buildDriverAuthentication(ride.getDriver().getUser());
// sharedRideService.forceCompleteRideRequestOfRide(
// new CompleteRideReqRequest(ride.getSharedRideId(),
...
// requestRepository.existsBySharedRideSharedRideIdAndStatusIn(
// ride.getSharedRideId(), activeStatuses);
// if (hasActiveRequests) {
// log.debug("Ride {} still has active requests; auto-completion skipped",
// ride.getSharedRideId());
// continue;
// }
...
// Authentication automationAuth =
// buildDriverAuthentication(ride.getDriver().getUser());
// sharedRideService.completeRide(new
// CompleteRideRequest(ride.getSharedRideId()), automationAuth);
// notifyDriverRideTransition(ride, "ride.auto.completed",
// "Ride auto-completed because all passengers finished and driver was
// inactive.");
// log.info("Auto-completed ride {} (started at {})", ride.getSharedRideId(),
// ride.getStartedAt());
// } catch (Exception ex) {
// log.warn("Auto-complete failed for ride {}: {}", ride.getSharedRideId(),
// ex.getMessage());
...
// private void notifyRideRequestTransition(SharedRideRequest request, String
// status, String message) {
// try {
// var riderUser = request.getRider().getUser();
// if (riderUser != null) {
// RiderMatchStatusNotification riderPayload =
// responseAssembler.toRiderLifecycleUpdate(
...
// try {
// var driver = request.getSharedRide() != null ?
// request.getSharedRide().getDriver() : null;
...
// private void notifyDriverRideTransition(SharedRide ride, String logKey,
// String message) {
// try {
...
// notificationService.sendNotification(
// driver.getUser(),
// logKey.contains("completed") ? NotificationType.RIDE_AUTO_COMPLETED :
// NotificationType.RIDE_AUTO_STARTED,
// "Ride update",
// message,
// null,
// Priority.MEDIUM,
